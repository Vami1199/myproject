               ...
               handler: function(grid, rowIndex, colIndex) {
                    ...
                    function(button, text) {
                        if (button == 'yes') {
                            ...
                            //获取选中行的pid属性
                            var id = rec.get('source.sourceId');
                            Ext.Ajax.request({
                                url: '/XJManager/base/deleteSource.do',
                                params: {
                                    'ids': id
                                },
                                method: 'POST',
            ...
            },
            {
                ...
                listeners: {
                    'click': function() {
                        ...
                        c.add(proForm);
                        proForm.form.reset();
                        proForm.getForm().url = '/XJManager/base/addSource.do';
                        proForm.show();
                        c.show();
                    }
                }
                ...
                listeners: {
                    'click': function() {
                        ...
                        if (record == undefined || record == '') {
                            ...
                            function(button, text) {
                                if (button == 'yes') {
                                    var url = "/XJManager/base/deleteSource.do";
                                    var param = "";
                                    for (var i = 0; i < record.length; i++) {
                                        param = param + record[i].get("source.sourceId") + "-";
                                    }
                                    Ext.Ajax.request({
                                        url: url,
                                        params: {
