...
function init_page(contextPath) {
	Ext.onReady(function() {
		...
		function change(val){
			if(val == 'O'){
				return XipLang.process_test_org_process;
			}else if(val == 'P'){
				return XipLang.process_test_pub_process;
			}else if(val == 'Y'){
				return XipLang.process_test_enable;
			}else if(val == 'N'){
				return XipLang.process_test_invalid;
			}else if(val == 'C'){
				return XipLang.process_test_copy;
			}
			return val;
		}		
		...
		function loadStore(){
			...
			if(appId == null || appId == "" || appId == "undefined"){
				Ext.Msg.alert(XipLang.xwf_lang_notice,XipLang.upgrade_select_app);
				return false;
			}
			store.load({
				params : {
		...
		var appl = new Ext.form.ComboBox({
			...
			fieldLabel		:	XipLang.process_test_select_app,
			emptyText		:	XipLang.process_test_choose,
			labelSeparator	:	':',
			selectOnFocus	:	true,
			triggerAction	:	'all',
		...
		var entity = new Ext.form.ComboBox({
			store			:	entityStore,
			width 			: 	document.body.clientWidth*(.2),
			fieldLabel		:	XipLang.process_test_entity_sel,
			labelSeparator	:	':',
			selectOnFocus	:	true,
			disabled		:	true,
			triggerAction	:	'all',
			...
			listeners		:	{
				'render' : function(){
					entity.setValue('ALL') ;
					entity.setRawValue(XipLang.process_test_all) ;
				},
				'select'	: 	function(){
					loadStore();
					loadStore2();
		...
		var formPanel = new Ext.form.FormPanel({
			title		:	XipLang.process_test_query_condition,
			width 		:	'100%',
			height 		: 	80,
			region 		: 	'north',
			labelAlign 	: 	"right",
...
//	    	 url : contextPath + '/commonQueryAction.do?method=getProcesses',
		...
		var cmArray = [] ;
		cmArray.push(sm);
		cmArray.push(new Ext.grid.RowNumberer({header:XipLang.xwf_lang_number,width:35}));
		cmArray.push({header: XipLang.process_test_entity_name,width: 250,align:'left',dataIndex: 'ENTITYNAME',sortable:true});
		cmArray.push({header: XipLang.xwf_lang_process_name,width: 350,align:'left',dataIndex: 'NAME',sortable:true});
		cmArray.push({header: XipLang.process_test_pro_status,width: 80,align:'left',dataIndex: 'ENABLEFLAG',sortable:true,renderer:change});
		cmArray.push({header: XipLang.process_test_pro_type,width: 100,align:'left',dataIndex: 'PROCESSCATEGORY',sortable:true,renderer:change});
		cmArray.push({header: XipLang.xwf_lang_org_name,width: 350,align:'left',dataIndex: 'ORGNAME',sortable:true});
		...
		var cm = new Ext.grid.ColumnModel(cmArray);
	 	...
	 	var pagebar = new Ext.PagingToolbar({
	  	  	...
	  	  	store		:	store,
	  	  	displayInfo	:	true,
	  	  	displayMsg	:	XipLang.process_test_page_no,
	  	  	emptyMsg	:	XipLang.process_test_no_display
	  	});			
		...
		var p1 = new Ext.Panel({
			title	:	XipLang.process_test_created_pro,
			tbar	:	new Ext.Toolbar([
				'-',
				XipLang.xwf_lang_process_name+':',
				new Ext.form.TextField({
					id				:	'pId' ,
					width			:	250,
					inputType		:	'text',
					fieldLabel		:	XipLang.xwf_lang_process_name,
					labelSeparator	:	':',
					listeners	:	{
				...
				'-',
				new Ext.Button({
					text	:	XipLang.process_test_start_pro,
					handler	:	function(){
						// 取得流程ID
						var processId ;
						var record_array = sm.getSelections() ;
						if(record_array.length == 0){
							Ext.Msg.alert(XipLang.xwf_lang_notice,XipLang.process_test_sel_ready_pro) ;
							return false ;
						}else{
...
//	    	 url : contextPath + '/commonQueryAction.do?method=getInstances',
		...
		var cmArray2 = [] ;
		cmArray2.push(sm2);
		cmArray2.push(new Ext.grid.RowNumberer({header:XipLang.xwf_lang_number,width:35}));
		cmArray2.push({header: XipLang.process_test_entity_name,width: 250,align:'left',dataIndex: 'ENTITYNAME',sortable:true});
		cmArray2.push({header: XipLang.process_test_entity_title,width: 350,align:'left',dataIndex: 'INSTANCETITLE',sortable:true,renderer:openMonitor});
		cmArray2.push({header: XipLang.process_test_instance_status,width: 80,align:'left',dataIndex: 'INSTANCESTATE',sortable:true,renderer:chanageState});
		cmArray2.push({header: XipLang.process_test_start_date,width: 130,align:'left',dataIndex: 'BEGINDATE',sortable:true});
		cmArray2.push({header: XipLang.process_test_end_date,width: 130,align:'left',dataIndex: 'ENDDATE',sortable:true});
		...
		cmArray2.push({header: "instanceCode",hidden:true,dataIndex: 'INSTANCECODE',sortable:true});
		cmArray2.push({header: "业务ID",hidden:true, dataIndex: 'BUSINESSID',sortable:true});
		cmArray2.push({header: XipLang.process_test_business_code,width: 150,align:'left',dataIndex: 'BUSINESSCODE',sortable:true});
		cmArray2.push({header: XipLang.process_test_business_name,width: 350,align:'left',dataIndex: 'BUSINESSNAME',sortable:true});
		...
		var p2 = new Ext.Panel({
			title	:	XipLang.process_test_run_pro,
			tbar	:	new Ext.Toolbar([
				'-',
				XipLang.process_test_entity_title+':',
				new Ext.form.TextField({
					id				:	'pId2',
			    	width			:	250,
			    	inputType		:	'text',
			    	fieldLabel		:	XipLang.process_test_entity_title,
			    	labelSeparator	:	':',
					listeners	:	{
						'specialkey':function(field,e){
							if (e.getKey() == Ext.EventObject.ENTER) {
				...
				'-',
				new Ext.Button({
					text	:	XipLang.process_test_submit_pro,
					handler	:	function(){
						// 取得流程ID
						var instanceCode ;
						var record_array = sm2.getSelections() ;
						if(record_array.length == 0){
							Ext.Msg.alert(XipLang.xwf_lang_notice,XipLang.process_select_instance) ;
							return false ;
						}else{
							Ext.each(record_array , function(r){
				...
				'-',
				new Ext.Button({
					text	:	XipLang.process_rollback_process,
					handler	:	function(){
						// 取得流程ID
						var instanceCode ;
						var record_array = sm2.getSelections() ;
...
//	    	 url : contextPath + '/commonQueryAction.do?method=getInstances',
		...
		var cmArray3 = [] ;
		cmArray3.push(sm3);
		cmArray3.push(new Ext.grid.RowNumberer({header:XipLang.xwf_lang_number,width:35}));
		cmArray3.push({header: XipLang.process_test_entity_name,width: 250,align:'left',dataIndex: 'ENTITYNAME',sortable:true});
		cmArray3.push({header: XipLang.process_test_entity_title,width: 350,align:'left',dataIndex: 'INSTANCETITLE',sortable:true,renderer:openMonitor});
		cmArray3.push({header: XipLang.process_test_instance_status,width: 80,align:'left',dataIndex: 'INSTANCESTATE',sortable:true,renderer:chanageState});
		cmArray3.push({header: XipLang.process_test_start_date,width: 130,align:'left',dataIndex: 'BEGINDATE',sortable:true});
		cmArray3.push({header: XipLang.process_test_end_date,width: 130,align:'left',dataIndex: 'ENDDATE',sortable:true});
		...
		cmArray3.push({header: "instanceCode",hidden:true,dataIndex: 'INSTANCECODE',sortable:true});
		cmArray3.push({header: "业务ID",hidden:true, dataIndex: 'BUSINESSID',sortable:true});
		cmArray3.push({header: XipLang.process_test_business_code,width: 150,align:'left',dataIndex: 'BUSINESSCODE',sortable:true});
		cmArray3.push({header: XipLang.process_test_business_name,width: 350,align:'left',dataIndex: 'BUSINESSNAME',sortable:true});		
		...
		var p3 = new Ext.Panel({
			title	:	XipLang.process_rollbacked_process,
			tbar	:	new Ext.Toolbar([
				'-',
				XipLang.process_test_entity_title+':',
				new Ext.form.TextField({
				...
				'-',
				new Ext.Button({
					text	:	XipLang.process_restart_process,
					handler	:	function(){
						// 取得流程ID
						var instanceCode ;
						var processId ;
...
//	    	 url : contextPath + '/commonQueryAction.do?method=getInstances',
		...
		var cmArray4 = [] ;
//		cmArray4.push(sm4);
		cmArray4.push(new Ext.grid.RowNumberer({header:XipLang.xwf_lang_number,width:35}));
		cmArray4.push({header: XipLang.process_test_entity_name,width: 250,align:'left',dataIndex: 'ENTITYNAME',sortable:true});
		cmArray4.push({header: XipLang.process_test_entity_title,width: 350,align:'left',dataIndex: 'INSTANCETITLE',sortable:true,renderer:openMonitor});
		cmArray4.push({header: XipLang.process_test_instance_status,width: 80,align:'left',dataIndex: 'INSTANCESTATE',sortable:true,renderer:chanageState});
		cmArray4.push({header: XipLang.process_test_start_date,width: 130,align:'left',dataIndex: 'BEGINDATE',sortable:true});
		cmArray4.push({header: XipLang.process_test_end_date,width: 130,align:'left',dataIndex: 'ENDDATE',sortable:true});
		...
		cmArray4.push({header: "instanceCode",hidden:true,dataIndex: 'INSTANCECODE',sortable:true});
		cmArray4.push({header: "业务ID",hidden:true, dataIndex: 'BUSINESSID',sortable:true});
		cmArray4.push({header: XipLang.process_test_business_code,width: 150,align:'left',dataIndex: 'BUSINESSCODE',sortable:true});
		cmArray4.push({header: XipLang.process_test_business_name,width: 350,align:'left',dataIndex: 'BUSINESSNAME',sortable:true});		
		...
		var p4 = new Ext.Panel({
			title	:	XipLang.process_finished_process,
			tbar	:	new Ext.Toolbar([
				'-',
				XipLang.process_test_entity_title+':',
				new Ext.form.TextField({
