(function(b) {
    ...
    a.config = {
        ...
        props: {
            ...
            props: {
            	...
            	other : {name:'other', label:'其他', value:'', editor:function(){return new myflow.editors.inputEditor();}}
    ...
    //给Raphael绑定事件  h:{},f:Raphael对象
    a.props = function(h, f) {
        var j = this,
        //绑定属性窗口的 拖动    尺寸变化 单击事件
...
//        e = c.find("table"),
        ...
        /*
         *Raphael 画布点击绑定事件
         *参数说明：n:Event,m:a.props(弹出窗口的属性配置),o:Raphael对象(例如：任务节点rect 连线 line)
         *modified：弹出Ext属性设置窗口
         */
        var d = function(n, m, o) {
        	 var createPanelType = o.getId().substring(0,4);
        	 if (i && i.getId() == o.getId()) {
                 return;
             }
        	 //切换属性窗口前，先保存之前panel属性的值
//           if(i){
//        	   var _tmpPropertyObj = o.getWfdata();
//        	   
//          	   //_tmpPropertyObj.wfdata = {'name':'哈哈','value':'我修改了！'};
//          }
      	    var _tmpform = app.eastPanel.down('form');
      	    if(_tmpform){
      	    	//保存数据
      	    	//移除panel
      		  app.eastPanel.remove(_tmpform);
      	    }
      	    //流程属性
        	if("0000" == createPanelType){
              	Wb.run({
            		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-panel',
            		success:function(scope,responseText){
            			app.eastPanel.add(scope.processForm);
            			propertyPanel = scope.processForm;
            			scope.init(m.wfdata);
            			scope.saveBeforeRemove = function(ps){
            				alert(ps);
            			};
            		},
        	...
        	}if("rect" == createPanelType){
        		//获取创建节点的类型
        		var _raphaelWfType = o.getWfType();
        		switch (_raphaelWfType){
        		//开始节点
        		case  'start':
        			Wb.run({
                		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-start',
                		success:function(scope,responseText){
                			app.eastPanel.add(scope.processStartForm);
                			propertyPanel = scope.processStartForm;
                			scope.init(m.wfdata);
                			scope.saveBeforeRemove = function(ps){
                				alert(ps);
                			};
        		...
        		case 'end':
        			Wb.run({
                		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-end',
                		success:function(scope,responseText){
                			app.eastPanel.add(scope.processEndForm);
                			propertyPanel = scope.processEndForm;
                			scope.init(m.wfdata);
                			scope.saveBeforeRemove = function(ps){
                				alert(ps);
                			};
        		...
        		case 'task':
        			Wb.run({
                		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-notice',
                		success:function(scope,responseText){
                			app.eastPanel.add(scope.processNoticeForm);
                			propertyPanel = scope.processNoticeForm;
                			scope.init(m.wfdata);
                			scope.saveBeforeRemove = function(ps){
                				alert(ps);
                			};
        		...
        		case 'fork':
        			Wb.run({
                		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-judge',
                		success:function(scope,responseText){
                			app.eastPanel.add(scope.processJudgeForm);
                			propertyPanel = scope.processJudgeForm;
                			scope.init(m.wfdata);
                			scope.saveBeforeRemove = function(ps){
                				alert(ps);
                			};
        		...
        		case 'copy':
        			Wb.run({
                		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-copy',
                		success:function(scope,responseText){
                			app.eastPanel.add(scope.processCopyForm);
                			propertyPanel = scope.processCopyForm;
                			scope.init(m.wfdata);
                			scope.saveBeforeRemove = function(ps){
                				alert(ps);
                			};
        	...
        	}else if('path' == createPanelType){
            		Wb.run({
                		url:'m?xwl=xip/wf/wf-designer/propertyComponent/process-line',
                		success:function(scope,responseText){
                			app.eastPanel.add(scope.processLineForm);
                			propertyPanel = scope.processLineForm;
                			scope.init(m.wfdata);
                			scope.saveBeforeRemove = function(ps){
                				alert(ps);
                			};
...
//        	var _tabPanel = Ext.create("Xip.wf.panel.ProcessPanel");
//        	app.eastPanel.add(_tabPanel);
            i = o;
...
//            }
    ...
    a.init = function(x, r) {
        ...
        //画布单击，显示属性div x = myflow y raphael对象
        b(x).click(function() {
        ...
        b(y).bind("addrect",
        function(j, c, k) {
        	//j 是 Html dom 的 Event对象 c.Event {type: "addrect", timeStamp: 1489570143059.....}
        	//c 画图类型  k 附加属性    如Raphael画直线paper.path("M130,30 L200,30 L160,90 z").attr({"fill": "#5DDEF4","stroke": "#2A6570","stroke-width": 2});  
        ...
        if (a.config.editable) {
            ...
            b(x).droppable({
                accept: ".state",
                drop: function(c, i) {
                	//Raphael画布对象改变，重新计算偏移量(并不准确....)
                    b(y).trigger("addrect", [i.helper.attr("type"), {
                        attr: {
                            x: i.helper.offset().left-195,
                            y: i.helper.offset().top-90
                        }
                    }])
                }
            });
            ...
            //给Raphael对象绑定事件 showproper 事件
            new a.props({}, y)
        }
        //根据初始化数据创建流程图
        /*
         *修改点， B.states[s].type 记录要创建Raphael对象的类型
         */
        if (r.restore) {
            var B = r.restore;
