			...
			success : function(response, options) {
				...
				if(response.flag == 0){
					...
					var fieldset = new Ext.form.FieldSet({
				        title		:	(Ext.isEmpty(XipLang)==true?'审批意见':XipLang.wf_open_task_audit_comment),
					    labelAlign	: 	'right',
					    layout		: 	'column',
					    labelWidth	:	80,
					    height	 	:	220,	
					...
					new Ext.Window({
						id			:	'batchWin',
					    title 		: 	'<font color=red><i>【'+(Ext.isEmpty(XipLang)==true?'注意：所选待办信息须为同一流程、同一流程版本、相同节点且下一通知/会签节点无须手动选人':XipLang.wf_batch_audit_notice)+'】</i></font>',
					...
					Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'所选待办信息须为同一流程、同一流程版本、相同节点,请确认！':XipLang.wf_batch_audit_note)) ;
					return false;
				}
			},
			failure : function(reponse, options) {
