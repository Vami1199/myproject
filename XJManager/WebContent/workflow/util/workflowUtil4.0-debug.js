...
document.write('<script src="' + contextPath + '/xip/js/xip.js" type="text/javascript" ></sc' + 'ript>');
	...
	if(!params){
		...
		if(typeof(params) != "object"){
			try{
				...
				Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),ex) ;
				return ;
			}
		}
	}
    ...
    Ext.Ajax.request({
    	...
    	success	: 	function(res, opts){
			...
			if(returnJson.flag == 0){	
				...
				Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"寻找可执行流程失败：":XipLang.xwf_lang_pro_effective)+returnJson.msg,function(){callback(str);});
			}else{	
				// 找到多个符合条件的流程
				 ...
				 var ds = Ext.create('Ext.data.Store', {
					...
					function choose(){
						...
						if(records.length==1){
							...
							Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"确认流程":XipLang.xwf_lang_confirm_pro),(Ext.isEmpty(XipLang)==true?"已选择流程：":XipLang.xwf_lang_confirm_pro)+"【"+record.get('processName')+"】,"+(Ext.isEmpty(XipLang)==true?"确定继续吗？":XipLang.wf_util_process_continue),function(e){
								if(e=="yes"){
									params.flag=0;
    ...
    Ext.Ajax.request({
    	...
    	success	: 	function(res, opts){
    		...
    		if(returnJson.flag == 0){
    			...
    			var _actName = '' ;
    			if(records.length > 0){
    				_actName = '<font color=\"red\">【'+records[0].actName+'】</font>' ;
    			}
    			// 定义store
    			...
    			function choose(){
    				...
    				if(records.length>0){
    					...
    					Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"确认审批人":XipLang.xwf_lang_confirm_auditor)+":【"+uesrNames+"】？",function(e){
    						if(e=="yes"){
    							t_window.close();
    			...
    			var grid = new Ext.grid.GridPanel({
    				...
    				columns		: 	[
					    ...
					    {text:'actName',	dataIndex:'actName',	hidden:true,width:200},
					    ...
					    {text:(Ext.isEmpty(XipLang)==true?'组织名':XipLang.xwf_lang_org_name),  dataIndex:'orgName',	width:200}
    				],
    				listeners:{
    					'rowdblclick':choose
    				}
    			...
    			var t_window = new Ext.Window({
    				...
    				modal	:	true,
    				closeAction	:	'close',
    				title	:	(Ext.isEmpty(XipLang)==true?'请选择节点审批人':XipLang.wf_global_audtior_title)+':'+_actName,
    				buttons	:	[
    				    {
			        		text:(Ext.isEmpty(XipLang)==true?'确定':XipLang.xwf_lang_btn_confirm),handler:choose 
			         	},{
    	...
    	success: function(res, opts){
			...
			if(returnJson.flag ==0){	
				...
				function choose(){
					...
					if(records.length==1){
						var record = records[0];
						Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"已选择流程：":XipLang.xwf_lang_confirm_pro)+"【"+record.get('processName')+"】,"+(Ext.isEmpty(XipLang)==true?"确定继续吗？":XipLang.wf_util_process_continue),function(e){
							if(e=="yes"){
								t_window.close();
...
function restartInstanceSubmit(instanceCode,processId,params,callback){
	restartInstance(instanceCode,processId,function(obj){
		if(obj.flag!=0){
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),obj.msg,function(){callback(obj);});
			return;
		}
		//重启成功，并提交
		submitInstance(instanceCode,params,callback);
...
function showMonitorPage(insCode){
	...
	var windowConfig = {'title':(Ext.isEmpty(XipLang)==true?'流程监控':XipLang.xwf_lang_pro_monitor), 'url':url, 'isMax':true};
...
function doBack(taskId, comment, callback){
	if(comment==null || comment==''){
		Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'请输入回退意见!':XipLang.xwf_lang_back_comment));
		return;
	}
...
function doReject(taskId, comment, callback){
	if(comment == null || comment == "" || comment == "undefined"){
		Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"请填写驳回意见！":XipLang.xwf_lang_reject_comment)) ;
		return false ; 
	}	
	// 等待框
	var msg = Ext.MessageBox.show({
		...
		if(returnJson.flag == 0){
			...
			var grid = new Ext.grid.GridPanel({
				...
				columns: [
				          	new Ext.grid.RowNumberer(),
						    {text:'ID',dataIndex:'userId',hidden:true},
					        {header: (Ext.isEmpty(XipLang)==true?'用户名':XipLang.xwf_lang_user_name), width:80, dataIndex: 'userName'},
					        {header: (Ext.isEmpty(XipLang)==true?'员工名':XipLang.xwf_lang_emp_name), width:100, dataIndex: 'empName'},
					        {header: (Ext.isEmpty(XipLang)==true?'组织名':XipLang.xwf_lang_org_name), width:170, dataIndex: 'orgName'}
				         ],
				listeners:{
					'rowdblclick':choose
				}
...
function complateBack(taskId, comment, backToActivityId, callback){
	if(comment == null || comment == "" || comment == "undefined"){
		Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"请填写回退意见!":XipLang.wf_global_back_comment)) ;
		return false ; 
	}	
	// 等待框
	var msg = Ext.MessageBox.show({
...
function chooseAllUsers(taskId, buttonCode, comment, callback){
	...
	var myGrid = Ext.create('Ext.grid.Panel',{
	    ...
	    tbar : mytbar ,
	    columns	: [
	        new Ext.grid.RowNumberer({header:(Ext.isEmpty(XipLang)==true?'序号':XipLang.xwf_lang_number),width:35}),
    ...
    var sys_lang_tmp_title1 = "请选择征询人";
    var sys_lang_tmp_title2 = "请选择委派人";
    if(!Ext.isEmpty(XipLang)){
    	 sys_lang_tmp_title1 = XipLang.xwf_lang_asker;
    	 sys_lang_tmp_title2 = XipLang.xwf_lang_signer;
    }
	var tttile = (buttonCode=="btnConsult" ? sys_lang_tmp_title1 :sys_lang_tmp_title2) ; 
