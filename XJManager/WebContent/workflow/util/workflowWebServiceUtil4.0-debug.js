...
function sendAjaxRequest(url,params,callback){
	...
	Ext.Ajax.request({
	    ...
	    failure: function ( result, request) {
	    	Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'请求提交失败，可能是网络或其他原因':XipLang.xwf_lang_request_error));
	    }
	});
};
...
function createInstanceByEntityCode(entityCode,params,loginName,callback){
	getProcessIdByEntityCode(entityCode,params,loginName,function(obj){
		if(obj.flag !=0){
			...
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),obj.msg);
			return; 
		}
...
function createInstanceByProcessId(processId,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?'根据流程Id创建流程实例....':XipLang.xwf_lang_id_create));
...
function createInstanceByProcessCode(processCode,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?'根据流程Code创建流程实例....':XipLang.xwf_lang_ins_create));
...
function getProcessIdByEntityCode(entityCode,params,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?'正在根据流程实体编码查询可执行流程....':XipLang.xwf_lang_pro_code_query));
    ...
    Ext.Ajax.request({
    	...
    	success: function(res, opts){
			...
			if(returnJson.flag ==0){
				...
				//Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),"找到可执行流程【"+returnJson.processName+"】",function(){callback(str);});
				if(callback){
					callback(str);
				}
				return;
				...
				Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),"寻找可执行流程失败："+returnJson.msg,function(){
					if(callback){
						callback(str);
					}
				});
				...
				var grid = Ext.create('Ext.grid.Panel', {
				    ...
				    columns:[
				             new Ext.grid.RowNumberer(),
							    ...
							    {text:(Ext.isEmpty(XipLang)==true?"流程编码":XipLang.xwf_lang_process_code),dataIndex:'processCode',width:130},
							    {text:(Ext.isEmpty(XipLang)==true?"流程名称":XipLang.xwf_lang_process_name),dataIndex:'processName',width:180}
							 ],
				    height: 200,
				    width: 400,
				...
				var t_window = new Ext.Window({
					...
					modal:true,
					closeAction:'close',
					title:(Ext.isEmpty(XipLang)==true?"找到多个流程,请选择一条":XipLang.wf_util_find_many_pro),
					buttons:[
					         {
					        	text:(Ext.isEmpty(XipLang)==true?"确定":XipLang.xwf_lang_btn_confirm),handler:choose
					         },{
					        	 text:(Ext.isEmpty(XipLang)==true?"取消":XipLang.xwf_lang_btn_cancel),handler:function(){
					        		 t_window.close(); 
					        	 }
					         }]
				});
				...
				function choose(){
					...
					if(records.length==1){
						...
						Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"确认流程":XipLang.xwf_lang_confirm_pro),"选择流程:【"+record.get('processName')+"】,确定继续吗？",function(e){
							if(e=="yes"){
						...
						Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"请先选中一条流程！":XipLang.xwf_lang_one_choose));
					}
				}
			}
    	...
    	failure: function(res, opts){
    		xt.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'请求提交失败，可能是网络或其他原因':XipLang.xwf_lang_request_error));
    	}
    });
...
function submitInstance(instanceCode,params,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"'流程实例提交中....":XipLang.xwf_lang_ins_submit));
	if(!params){params = {}};
    ...
    Ext.Ajax.request({
    	...
    	success: function(res, opts){
	    	...
	    	if(returnJson.flag == 0){
				...
				Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"流程实例提交成功":XipLang.wf_global_instance_success),function(){
					if(callback){
						callback(str);
					}
				});
				...
				var str = {flag:1,msg:returnJson.msg};
				Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'流程实例提交失败,失败原因:':XipLang.wf_global_instance_failure)+returnJson.msg,function(){
					if(callback){
						callback(str);
					}
				});
				...
				var grid = new Ext.grid.GridPanel({
					...
					columns: [
							    ...
							    {text:(Ext.isEmpty(XipLang)==true?'用户名':XipLang.xwf_lang_user_name), dataIndex:'userName',	width:80},
							    {text:(Ext.isEmpty(XipLang)==true?'员工名':XipLang.xwf_lang_emp_name),  dataIndex:'empName',	width:100},
							    {text:(Ext.isEmpty(XipLang)==true?'组织名':XipLang.xwf_lang_org_name),  dataIndex:'orgName',	width:170}
					         ],
					listeners:{
						'rowdblclick':choose,
				...
				var t_window = new Ext.Window({
					...
					modal:true,
					closeAction:'close',
					title:(Ext.isEmpty(XipLang)==true?'请选择节点审批人':XipLang.wf_global_audtior_title),
					buttons:[
					         {
					        	text:(Ext.isEmpty(XipLang)==true?'确定':XipLang.xwf_lang_btn_confirm),handler:choose
					         },{
				...
				function choose(){
					...
					if(records.length>0){
						...
						Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'确认选择审批人':XipLang.wf_confirm_sel_auditor)+":【"+uesrNames+"】",function(e){
							if(e=="yes"){
								t_window.close();
						...
						Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'请先选择审批人':XipLang.xwf_lang_auditor_choose));
					}
				}
			}
    	},
...
function revokeByInstanceCode(instanceCode,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?'流程撤回中....':XipLang.xwf_lang_pro_recall));
...
function restartInstance(instanceCode,processId,loginName,callback){
	(Ext.isEmpty(XipLang)==true?"流程重启中....":XipLang.xwf_lang_pro_restart)
...
function restartInstanceSubmit(instanceCode,processId,params,loginName,callback){
	restartInstance(instanceCode,processId,loginName,function(obj){
		if(obj.flag!=0){
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),obj.msg,function(){
				if(callback){
					callback(obj);
				}
			});
...
function stopProcess(instanceId,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"正在终止流程....":XipLang.xwf_lang_pro_stop));
...
function completeWorkItem(taskId,lineCode,comment,loginName,callback){
	...
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?'正在完成待办....':XipLang.xwf_lang_complete_task));
	...
	sendAjaxRequest(url,params,function(returnJson){
		...
		if(returnJson.flag == 0){
			...
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'待办处理成功！':XipLang.wf_util_task_finished),function(){
				if(callback){
					callback(str);
				}
			});
			...
			var str = {flag:1,msg:returnJson.msg};
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'待办处理失败,失败原因:':XipLang.wf_util_task_failure)+returnJson.msg,function(){
				if(callback){
					callback(str);
				}
			});
			...
			var selModel = Ext.create('Ext.selection.CheckboxModel',{
		 	...
		 	var selUser = Ext.create('Ext.form.field.Text',{
		        ...
		        inputType 		: 	'text',
		        labelSeparator 	:	':',
		        fieldLabel		:	(Ext.isEmpty(XipLang)==true?"姓名":XipLang.wf_global_person_name),
		        labelWidth		: 	50,
		        labelAlign		:	'right',
		        listeners 		: 	{
			...
			var grid = new Ext.grid.GridPanel({
				...
				columns	: [
				          	new Ext.grid.RowNumberer(),
						    {text:'ID',dataIndex:'userId',hidden:true},
						    {header: (Ext.isEmpty(XipLang)==true?'用户名':XipLang.xwf_lang_user_name), width:100, dataIndex: 'userName'},
					        {header: (Ext.isEmpty(XipLang)==true?'员工名':XipLang.xwf_lang_emp_name), width:150, dataIndex: 'empName'},
					        {header: (Ext.isEmpty(XipLang)==true?'组织名':XipLang.xwf_lang_org_name), width:450, dataIndex: 'orgName'}
				         ],
				listeners:{
					'rowdblclick':choose,
					'afterrender' : function(){
			...
			var t_window = new Ext.Window({
				...
				modal		:	true,
				closeAction	:	'close',
				title		:	(Ext.isEmpty(XipLang)==true?'请选择审批人':XipLang.task_util_query_auditor),
				buttonAlign	:	'right',
				buttons:[
				         {
				        	text:(Ext.isEmpty(XipLang)==true?'选择':XipLang.xwf_lang_selecting),handler:choose
				         },{
				        	 text:(Ext.isEmpty(XipLang)==true?'取消':XipLang.xwf_lang_btn_cancel),handler:function(){
				        		 t_window.close(); 
				        	 }
...
function doConsult(taskId, loginName, callback, comment){
	...
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?'正在提交请求.......':XipLang.xwf_lang_submit_request));
	sendAjaxRequest(url,params,function(returnJson){
		if(returnJson.flag == 1){
			var str = {flag:1,msg:returnJson.msg};
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'查询征询人出错,失败原因:':XipLang.wf_global_consulate_query)+returnJson.msg,function(){
				if(callback){
					callback(str);
				}
			});
			...
			var grid = new Ext.grid.GridPanel({
				...
				columns	: [
						    ...
						    {text:'ID',dataIndex:'userId',hidden:true},
						    {header: (Ext.isEmpty(XipLang)==true?'用户名':XipLang.xwf_lang_user_name), width:100, dataIndex: 'userName'},
					        {header: (Ext.isEmpty(XipLang)==true?'员工':XipLang.xwf_lang_emp_name), width:150, dataIndex: 'empName'},
					        {header: (Ext.isEmpty(XipLang)==true?'组织':XipLang.xwf_lang_org_name), width:400, dataIndex: 'orgName'}
				         ],
				listeners:{
					'rowdblclick':choose,
					'afterrender' : function(){
			...
			var t_window = new Ext.Window({
				...
				modal		:	true,
				closeAction	:	'close',
				title		:	(Ext.isEmpty(XipLang)==true?'请选择被征询人':XipLang.wf_need_sel_consulate),
				buttonAlign	:	'right',
				buttons		:	[
				         {
				        	text:(Ext.isEmpty(XipLang)==true?'选择':XipLang.xwf_lang_selecting),handler:choose
			...
			function choose(){
				...
				if(records.length>0){
					...
					Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"确定选择征询人":XipLang.wf_confirm_consulate_choose)+":【"+uesrNames+"】",function(e){
						if(e=="yes"){
							t_window.close();
							...
							sendAjaxRequest(url,subParams,function(returnJson){
								if(returnJson.flag == 0){
									var str = {"flag":"0","msg":""};
									Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'征询成功！':XipLang.wf_ask_suggestion_success),function(){
										if(callback){
											callback(str);
										}
									});
									...
									var str = {"flag":"1","msg":returnJson.msg};
									Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'征询失败: ':XipLang.wf_ask_suggestion_failure)+returnJson.msg,function(){
										if(callback){
											callback(str);
										}
									});
		...
		if(returnJson.flag == 1){
			...
			var grid = new Ext.grid.GridPanel({
				...
				columns	: [
				          	new Ext.grid.RowNumberer(),
						    {text:'ID',dataIndex:'userId',hidden:true},
						    {text:(Ext.isEmpty(XipLang)==true?'用户名':XipLang.xwf_lang_user_name), dataIndex:'userName',	width:120},
						    {text:(Ext.isEmpty(XipLang)==true?'员工名':XipLang.xwf_lang_emp_name),  dataIndex:'empName',	width:150},
						    {text:(Ext.isEmpty(XipLang)==true?'组织名':XipLang.xwf_lang_org_name),  dataIndex:'orgName',	width:400}
				         ],
				listeners:{
					'rowdblclick':choose,
					'afterrender' : function(){
			...
			var t_window = new Ext.Window({
				...
				modal		:	true,
				closeAction	:	'close',
				title		:	(Ext.isEmpty(XipLang)==true?'请选择委派人':XipLang.xwf_lang_signer), 
				buttonAlign	:	'right',
				buttons		:	[
				         {
				        	text:(Ext.isEmpty(XipLang)==true?'选择':XipLang.xwf_lang_selecting),handler:choose
			...
			function choose(){
				...
				if(records.length>0){
					...
					Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"确定选择委派人":XipLang.wf_appiont_confirm_person)+":【"+uesrNames+"】",function(e){
						if(e=="yes"){
							t_window.close();
							if(uesrNames && uesrNames!=''){
								uesrNames = encodeURI(uesrNames);
							...
							sendAjaxRequest(url,subParams,function(returnJson){
								if(returnJson.flag == 0){
									var sys_lang_appiont = "委派成功！";
									if(!Ext.isEmpty(XipLang)){
										sys_lang_appiont = XipLang.wf_appionter_success;
									}
									var str = {"flag":"0","msg":sys_lang_appiont};
									Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),sys_lang_appiont,function(){
										if(callback){
											callback(str);
										}
									});
									...
									var sys_lang_appiont = "委派失败";
										if(!Ext.isEmpty(XipLang)){
											sys_lang_appiont = XipLang.wf_appionter_failure;
										}
									var str = {"flag":"1","msg":returnJson.msg};
									Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),sys_lang_appiont+':'+returnJson.msg,function(){
										if(callback){
											callback(str);
										}
									});
...
function doBack(taskId,backToActivityId,comment,loginName,callback){
	...
	sendAjaxRequest(url,params,function(returnJson){
		...
		if(returnJson.flag == 1){
			var str = {flag:1,msg:returnJson.msg};
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),'查询回退节点出错,失败原因:'+returnJson.msg,function(){
				if(callback){
					callback(str);
				}
			});
			...
			var grid = new Ext.grid.GridPanel({
				...
				columns: [
						    ...
						    {header: (Ext.isEmpty(XipLang)==true?'节点编码':XipLang.xwf_lang_node_code), width:80, dataIndex: 'activityCode'},
					        {header: (Ext.isEmpty(XipLang)==true?'节点名称':XipLang.xwf_lang_node_name), width:100, dataIndex: 'activityName'}
				         ],
				listeners:{'rowdblclick':choose}
			});  
			var t_window = new Ext.Window({
				...
				modal:true,
				closeAction:'close',
				title:(Ext.isEmpty(XipLang)==true?"节点列表":XipLang.wf_activity_list),
				buttons:[
				         {
				        	text:(Ext.isEmpty(XipLang)==true?"确定":XipLang.xwf_lang_btn_confirm),handler:choose
				         },{
			...
			function choose(){
				...
				if(records.length>0){
					...
					Ext.MessageBox.confirm((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'确认选择节点':XipLang.wf_confirm_choose_act)+":【"+activityIdName+"】",function(e){
						if(e=="yes"){
							t_window.close();
							...
							sendAjaxRequest(url,subParams,function(returnJson){
								...
								if(returnJson.flag == 0){
									var str = {flag:0,taskId:taskId};
									Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"退回成功":XipLang.wf_rollback_success),function(){
										if(callback){
											callback(str);
										}
									});
									...
									var str = {flag:1,msg:returnJson.msg};
									Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?"退回失败,失败原因:":XipLang.wf_rollback_failure
											
									)+returnJson.msg,function(){
										if(callback){
											callback(str);
										}
									});
...
function closeConsultTask(taskId,comment,loginName,callback){
	...
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"正在执行征询反馈....":XipLang.wf_executing_feedback));
...
function revokeConsultTask(taskId,comment,loginName,callback){
	...
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"正在撤销征询....":XipLang.wf_canceling_consulate));
...
function doRejectProcess(taskId,approverComment,loginName,callback){
	...
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"正在驳回流程....":XipLang.wf_rejecting_process));
...
function closeCopyToTask(taskId,loginName,callback){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"正在关闭抄送节点....":XipLang.wf_closing_mail_node));
...
function showMonitorPage(insCode,callback){
	if(insCode==null || insCode=='undefined' || insCode==''){
		Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),(Ext.isEmpty(XipLang)==true?'请指定实例编码！':XipLang.xwf_lang_appoint_code));
		return;
	}	
	...
	var extWindow = new Ext.Window({
		id:winId,
	    title: (Ext.isEmpty(XipLang)==true?"流程监控":XipLang.xwf_lang_pro_monitor),
	    height: 500,
	    width: 770,
	    layout: 'fit',
	    modal: true,
...
function getProcIdByCode(processCode){
	Ext.Msg.wait((Ext.isEmpty(XipLang)==true?"正在查询流程信息......":XipLang.wf_querying_process));
	...
	var callback = function(resp){
		if(resp.flag == "0"){
			...
			Ext.Msg.alert((Ext.isEmpty(XipLang)==true?"提示":XipLang.xwf_lang_notice),resp.msg) ;
		}
	}
	sendAjaxRequest(url,params,callback);
	Ext.Msg.hide();
